...
Item {
    ...
    onVisibleChanged: {
        if (currentPage) {
            internal.setPageStatus(currentPage, visible ? 2 : 0/*PageStatus.Active : PageStatus.Inactive*/);
            if (visible)
    ...
    QtObject {
        ...
        function setPageStatus(page, status) {
            if (page.status !== undefined) {
                if (status == 2/*PageStatus.Active*/ && page.status == 0/*PageStatus.Inactive*/)
                    page.status = 1/*PageStatus.Activating*/;
                else if (status == 0/*PageStatus.Inactive*/ && page.status == 2/*PageStatus.Active*/)
                    page.status = 3/*PageStatus.Deactivating*/;
        ...
        Item {
            ...
            function pushEnter(immediate, orientationChanges) {
                ...
                if (root.visible && immediate)
                    internal.setPageStatus(page, 2 /*PageStatus.Active*/);
            }
            ...
            function pushExit(replace, immediate, orientationChanges) {
                ...
                if (root.visible && immediate)
                    internal.setPageStatus(page, 0 /*PageStatus.Inactive*/);
                if (replace) {
                    if (immediate)
                        cleanup();
                    else
            ...
            function transitionStarted() {
                ...
                if (root.visible)
                    internal.setPageStatus(page, (state == "") ? 1 : 3/*PageStatus.Activating : PageStatus.Deactivating*/);
            }
            ...
            function transitionEnded() {
                if (state != "")
                    state = "Hidden";
                if (root.visible)
                    internal.setPageStatus(page, (state == "") ? 2 : 0/*PageStatus.Active : PageStatus.Inactive*/);
            ...
            function cleanup() {
                if (page.status === 2/*PageStatus.Active*/)
                    internal.setPageStatus(page, 0/*PageStatus.Inactive*/);
                if (owner !== container) {
